...
#PROBLEM: I have to put a lower target x and a very low tollerance. 
...
sub = rospy.Subscriber("/odom", Odometry, move_rot_callback)
...
while not rospy.is_shutdown():
    ...
    if distance_x > 0.00001 and state < 1 :
        
        
        vel = kL * distance_x*1000
        if abs(vel) > v_max:
        ...
        state = state +1
        if distance_x_return < 0.00001 and state >1:
        ...
        if angular_deviation < 0.05 and state > 1 and state_1 > 1 :
